...
namespace Sort
{
    ...
    public class Sort
    {
        ...
        /// <returns></returns>
        protected static bool Less<T>(T v, T w, Comparer<T> c)
        {
        ...
        /// <param name="i">需要交换的第一个元素。</param>
        /// <param name="j">需要交换的第二个元素。</param>
        protected static void Exch<T>(T[] a, int i, int j)
        {
            T t = a[i];
            a[i] = a[j];
            a[j] = t;
        ...
        /// <param name="c">比较器。</param>
        /// <returns>有序则返回 true，否则返回 false。</returns>
        public static bool IsSorted<T>(T[] a, Comparer<T> c)
        {
            for (int i = 1; i < a.Length; ++i)
            {
                if (Less(a[i], a[i - 1], c))
        ...
        /// <param name="c">比较器。</param>
        /// <returns>有序则返回 true，否则返回 false。</returns>
        public static bool IsSorted<T>(T[] a, int lo, int hi, Comparer<T> c)
        {
            for (int i = lo + 1; i <= hi; ++i)
            {
                if (Less(a[i], a[i - 1], c))
