...
namespace SymbolTable
{
        ...
        where TKey : IComparable<TKey>
    {
        ...
        /// </summary>
        /// <value>记录 <see cref="Get(TKey)"/> 用时的 <see cref="Stopwatch"/>。</value>
        public Stopwatch GetTimer { get; set; }
        /// <summary>
        /// 记录 <see cref="Put(TKey, TValue)"/> 用时的 <see cref="Stopwatch"/>。
        /// </summary>
        /// <value>记录 <see cref="Put(TKey, TValue)"/> 用时的 <see cref="Stopwatch"/>。</value>
        ...
        public int Rank(TKey key)
        {
            ...
            {
                var mid = lo + (hi - lo) / 2;
                var compare = this.keys[mid].CompareTo(key);
                ArrayVisit++;
                if (compare > 0)
                    hi = mid - 1;
                else if (compare < 0)
