namespace _3._2._11
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //
            // 反证：
            // 也可以这样考虑，假设序列 A 可以构造出一棵最坏情况下的二叉树，插入顺序为 x1...xn
            // 假设存在与 A 顺序不同的序列 B，它构造出的二叉树与 A 的相同。
            // 由于 A 和 B 的元素相同，因此 A 必然可以通过有限次元素交换得到 B。
            // 根据最坏情况下的二叉搜索树的性质，A 中的元素 xn 必然满足 x1~xn-1 的所有关系。
            // 例如，假设 x1 > x2， 则 x3 也必然满足 x1 > x3，即后面的元素必然满足前面元素的关系。
            // 于是 A 存在关系序列 r1 ... rn-1，只有满足这样序列的输入才能构造出对应的二叉树。
